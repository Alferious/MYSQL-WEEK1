...
CREATE TABLE jurisdiction_hierarchy (
	hierachy_id INT AUTO_INCREMENT,
    area VARCHAR(30),
    area_type VARCHAR(30),
    region_district_name VARCHAR(40) NULL,
    PRIMARY KEY(hierachy_id)
    );
...
CREATE TABLE villages(
	district_id INT,
    village_id INT,
    villa_id VARCHAR(10) PRIMARY KEY
    );

CREATE TABLE district_population AS
SELECT villa_id,total_population
FROM region,village_locations,villages;

CREATE TABLE district_summary AS
SELECT villa_id,total_population,beneficiaries,beneficiary_type
FROM village_locations,beneficiary_partner_data,villages
WHERE beneficiary_type = 'Households';

-- 2. PARTNER SUMMARY
CREATE TABLE partner_summary AS 
SELECT partner,villa_id
FROM beneficiary_partner_data,district_summary;
